...
const getRouteLevel = ({ avgRiskScore, highRiskPoints, mediumRiskPoints }) => {
  if (avgRiskScore >= 70 || highRiskPoints >= 3) {
    return "high";
  }

  if (avgRiskScore >= 45 || mediumRiskPoints >= 4) {
    return "medium";
  }
...
const calculateRouteRiskFromPredictions = (predictions) => {
  ...
  return {
    ...
    route_risk_level: getRouteLevel({
      avgRiskScore,
      highRiskPoints,
      mediumRiskPoints,
    }),
  ...
  try {
    ...
    const scoredRoutes = routes.map((route, index) => {
      ...
      return {
        ...
        route_risk_level: risk.route_risk_level,
        sampled_points_count: group.sampledCoordinates.length,
        high_risk_points: risk.high_risk_points,
        medium_risk_points: risk.medium_risk_points,
        geometry: route.geometry?.coordinates || [],
        sampled_predictions: routePredictions,
      };
    });

    scoredRoutes.sort((a, b) => {
      if (a.avg_risk_score !== b.avg_risk_score) {
        return a.avg_risk_score - b.avg_risk_score;
      }
